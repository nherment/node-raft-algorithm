...
Server.prototype.appendEntries = function(term,
  ...
  var self = this
  var ack = false
  if(term < this._state.term() || !this._matchTerm(prevLogIndex, prevLogTerm)) {
    ack = false
    console.log('ack false:', term, this._state.term(), prevLogIndex, prevLogTerm)
  } else {
    ack = true
    if(this._state.term() < term) {
      this._state.term(term)
      this._state.votedFor(null)
      this._state.leaderId(leaderId)
    }
    _.each(entries, function(entry) {
      self._state.appendLog(entry)
    })
...
Server.prototype._matchTerm = function(index, term) {
  ...
  if(log) {
    ...
    return true // ack on initial entries
  } else {
    return false
  }
