...
Server.prototype.requestVote = function(term,
  ...
  var currentTerm = this._state.term()
  if(term < currentTerm) {
    voteGranted = false
  } else if((term > currentTerm || !this._state.votedFor()) && lastLogTerm >= this._state.lastApplied()) {
    voteGranted = true
    this._state.term(term)
    this._state.votedFor(candidateId)
    this._state.role('follower')
  }
  logger.info('node', this._state.id(),
              'received vote request from candidate', candidateId,
              'with new term', term, 'while the current term is',  currentTerm,
              'and', voteGranted ? 'granted' : 'denied', 'its vote')
  callback(undefined, {
    term: this._state.term(),
